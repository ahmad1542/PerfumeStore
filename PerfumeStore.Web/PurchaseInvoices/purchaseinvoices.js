function setTableHeader() {
  ...
  if (!thead) return;
  thead.innerHTML = `<tr><th>#</th><th>Date</th><th>Supplier</th><th>AmountPaid</th><th>HasDebt</th><th style="text-align:right;">Actions</th></tr>`;
}
...
async function loadList(searchText = "") {
  ...
  list.forEach((x, index) => {
    ...
    cells.push(escapeHtml((x.supplier?.name ?? x.Supplier?.Name ?? x.supplierName ?? x.SupplierName ?? x.supplierPhone ?? x.SupplierPhone ?? '-')));
    cells.push(escapeHtml(x.amountPaid ?? x.AmountPaid ?? 0));
    cells.push(escapeHtml((x.debt != null || x.Debt != null) ? 'Yes' : 'No'));
